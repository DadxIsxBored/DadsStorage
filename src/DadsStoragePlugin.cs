...
using DadsStorage.APIs.MUC.MUCSrc.Patches.Compatibility;
using BepInEx.Logging;
using JetBrains.Annotations;
using ServerSync;

namespace DadsStorage;

[BepInPlugin(ModGUID, ModName, ModVersion)]
[BepInDependency("com.dadisbored.dadsepi", BepInDependency.DependencyFlags.SoftDependency)]
...
public class DadsStoragePlugin : BaseUnityPlugin
{
    ...
    public static readonly ManualLogSource DadsStorageLogger = BepInEx.Logging.Logger.CreateLogSource(ModName);
    ...
    internal static readonly CustomSyncedValue<string> DadsStorageContainerData = new(ConfigSync, "dadsstorageData", "");
    internal static readonly CustomSyncedValue<string> CraftyContainerGroupsData = new(ConfigSync, "dadsstorageGroupsData", "");

    //
    internal static Dictionary<string, object>? yamlData;
    internal static Dictionary<string?, HashSet<string?>> groups = null!;
    internal static DadsStoragePlugin self = null!;
    ...
    public void Awake()
    {
        ...
        DadsStorageContainerData.ValueChanged += OnValChangedUpdate; // check for file changes
        DadsStorageContainerData.AssignLocalValue(File.ReadAllText(yamlPath));
    ...
    public void Start()
    {
        ...
        Log.Init(DadsStorageLogger);

        if (!MUCCompat.DoNotPatch && Chainloader.PluginInfos.ContainsKey("mkz.itemdrawers"))
        {
            try
            {
                _harmony.PatchAll(typeof(ItemDrawerCompat));
            }
            catch (Exception e)
            {
                DadsStorageLogger.LogWarning($"Couldn't apply the ItemDrawers compatibility patches: {e.Message}");
            }
        }
    }
    ...
    {
        Assembly assembly = Assembly.GetExecutingAssembly();
        string resourceName = "DadsStorage.Example.yml";
    ...
    private void Update()
    {
        ...
        {
#if DEBUG
                DadsStorageLogger.LogError("Taking input");
#endif
            Functions.TryStore();
        }
    ...
    private void ReadYamlFiles(object sender, FileSystemEventArgs e)
    {
        ...
        try
        {
            DadsStorageLogger.LogDebug("ReadConfigValues called");
            DadsStorageContainerData.AssignLocalValue(File.ReadAllText(yamlPath));
        }
        catch
        {
            DadsStorageLogger.LogError($"There was an issue loading your {yamlFileName}");
            DadsStorageLogger.LogError("Please check your entries for spelling and format!");
        }
    }

    private static void OnValChangedUpdate()
    {
        DadsStorageLogger.LogDebug("OnValChanged called");
        try
        {
            YamlUtils.ReadYaml(DadsStorageContainerData.Value);
            YamlUtils.ParseGroups();
            Functions.ClearRangeCache();
        }
        catch (Exception e)
        {
            DadsStorageLogger.LogError($"Failed to deserialize {yamlFileName}: {e}");
        }
    }
    ...
    private static Texture2D loadTexture(string name)
    {
        ...
        LoadImageBytes(texture, ReadEmbeddedFileBytes("assets." + name));


        return texture!;
    }

    private static void LoadImageBytes(Texture2D texture, byte[] bytes)
    {
        MethodInfo loadImage = typeof(ImageConversion).GetMethod("LoadImage", new[]
        {
            typeof(Texture2D), typeof(byte[]), typeof(bool)
        });
        loadImage?.Invoke(null, new object[] { texture, bytes, false });
    }
