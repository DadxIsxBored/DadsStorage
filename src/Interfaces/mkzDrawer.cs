...
public sealed class mkzDrawer(MkzItemDrawers_API.Drawer _drawer) : IContainer
{
    ...
    internal static void LogDebug(string s) => DadsStorageLogger.LogDebug(s);
    ...
    public int TryStore()
    {
        ...
        for (int j = items.Count - 1; j >= 0; --j)
        {
            ...
            if (DadsEPICompat.IsLoaded())
            {
                List<ItemDrop.ItemData> quick = DadsEPICompat.GetQuickSlotsItems();
