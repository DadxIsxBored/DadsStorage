...
public class Boxes
{
    ...
    internal static void AddContainer(Container container)
    {
        if (!Containers.Contains(container))
        {
            ...
            DadsStorageLogger.LogDebug($"Added container {container.name} to list");
        }
    ...
    internal static void RemoveContainer(Container container)
    {
        if (Containers.Contains(container))
        {
            ...
            if (container)
                DadsStorageLogger.LogDebug($"Removed container {container.name} from list");
        }
    ...
    internal static List<IContainer> GetNearbyContainers<T>(T gameObject, float rangeToUse) where T : Component
    {
        ...
        foreach (Container container in Containers)
        {
            ...
            DadsStorageLogger.LogDebug($"Distance to container {container.name} is {distance}m, within the range of {rangeToUse}m set to store items for this chest");
    ...
    public static bool CanItemBeStored(string container, string prefab)
    {
        if (yamlData == null)
        {
            DadsStorageLogger.LogError("yamlData is null.");
            return false;
        }
        ...
        {
            DadsStorageLogger.LogError($"Unable to cast containerData for container '{container}' to Dictionary<object, object>.");
            return false;
        }

        List<object> excludeList = GetValueCaseInsensitive(containerData, "exclude") as List<object> ?? new List<object>();
        List<object> includeOverrideList = GetValueCaseInsensitive(containerData, "includeOverride") as List<object> ?? new List<object>();
    ...
    public static List<string?> GetExcludedPrefabs(string container)
    {
        if (yamlData != null && yamlData.TryGetValue(container, out object containerData))
        {
            ...
            if (containerInfo != null && GetValueCaseInsensitive(containerInfo, "exclude") is { } excludeData)
            {
    ...
    internal static object? GetValueCaseInsensitive(Dictionary<object, object> dict, string key)
    {
