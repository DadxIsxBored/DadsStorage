...
public class GroupUtils
{
    ...
    public static bool IsGroupDefined(string? groupName)
    {
        if (yamlData == null)
        {
            DadsStorageLogger.LogError("yamlData is null. Make sure that your YAML file is not empty or to call DeserializeYamlFile() before using IsGroupDefined.");
            return false;
        }
        ...
        if (yamlData.ContainsKey("groups"))
        {
            ...
            {
                DadsStorageLogger.LogError("Unable to cast groupsData to Dictionary<object, object>.");
            }
        }
