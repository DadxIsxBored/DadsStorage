...
public class MiscFunctions
{
    ...
    {
        DadsStorageLogger.LogDebug($"(ConsumeResourcesPatch) Have {totalAmount}/{totalRequirement} {reqName} in player inventory");
    }
        ...
        foreach (GameObject gameObject in __instance.m_items.Where(x => x.GetComponentInChildren<ItemDrop>() != null))
        {
            ...
            if (drop != null)
            {
                ...
                foreach (string? groupName in groupNames)
                {
                    ...
                    {
                        #if DEBUG
                        DadsStoragePlugin.DadsStorageLogger.LogDebug($"(CreatePredefinedGroups) Adding {itemDrop.m_itemData.m_dropPrefab.name} to {groupName}");
                        #endif
    ...
    private static void AddItemToGroup(string? groupName, ItemDrop itemDrop)
    {
        ...
        DadsStorageLogger.LogDebug($"(CreatePredefinedGroups) Added {prefabName} to {groupName}");
    }
